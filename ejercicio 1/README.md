...
# 🐳 Práctica: Despliegue de clúster K3s single-node y servicio NGINX

Este repositorio contiene las evidencias correspondientes a la práctica de despliegue de un clúster **K3s en modo single-node**, así como el despliegue de un servicio **NGINX** con **2 réplicas**. También se valida el estado del clúster y sus recursos con la herramienta **K9s** como en el punto 3.1.

---

## ⚙️ Instalación de K3s (modo single-node)

Se ha instalado **K3s** en una única máquina utilizando el parámetro `--cluster-init`.

### 📸 Evidencias:

- `images/instalacio.png` ➝ Instalación de K3s en nodo único.
- `images/instalacio_slave.png` ➝ (opcional) muestra redundante del nodo como worker para futura ampliación.

---

## 📦 Despliegue de NGINX con 2 réplicas

Se ha aplicado un manifiesto YAML con un `Deployment` de **NGINX** con 2 réplicas y un `Service` de tipo LoadBalancer.
...
- `images/2 replicas funcionando.png` ➝ Ambas réplicas ejecutándose correctamente.
- `images/svc.png` ➝ Servicio creado y escuchando en puerto externo (en `pending` si no hay LoadBalancer físico disponible).

---

## 🖥️ Validación con K9s

Se ha instalado y usado **K9s** para verificar visualmente el estado de los pods y del clúster.

### 📸 Evidencias:

- `images/k9s.png` ➝ Visualización del estado de los pods en ejecución.
- `images/exportar_config.png` ➝ Exportación de configuración desde K9s.
- `images/funcionament.png` ➝ Comprobación visual del acceso al servicio.
